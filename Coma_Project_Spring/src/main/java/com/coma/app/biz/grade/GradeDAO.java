...
public class GradeDAO {
	//데이터 추가
	private final String INSERT = "INSERT INTO GRADE(GRADE_NUM,GRADE_PROFILE,GRADE_NAME,GRADE_MIN_POINT,GRADE_MAX_POINT)\r\n"
			+ "VALUES((SELECT NVL(MAX(GRADE_NUM),0)+1 FROM GRADE),?,?,?,?)";
	...
	public boolean insert(GradeDTO gradeDTO) {
		...
		try {
			...
			pstmt.setString(1, gradeDTO.getGrade_profile());
			pstmt.setString(2, gradeDTO.getGrade_name());
			pstmt.setInt(3, gradeDTO.getGrade_min_point());
			pstmt.setInt(4, gradeDTO.getGrade_max_point());
	...
	public GradeDTO selectOne(GradeDTO gradeDTO){
		...
		try {
			...
			pstmt.setInt(1, gradeDTO.getGrade_num());
			
			ResultSet rs = pstmt.executeQuery();
			if(rs.next()) {
				System.out.println("grade.GradeDAO.selectOne 검색 성공");
				data = new GradeDTO();
				data.setGrade_num(rs.getInt("GRADE_NUM"));
				data.setGrade_name(rs.getString("GRADE_NAME"));
				data.setGrade_profile(rs.getString("GRADE_PROFILE"));
				data.setGrade_min_point(rs.getInt("GRADE_MIN_POINT"));
				data.setGrade_max_point(rs.getInt("GRADE_MAX_POINT"));
			}
		} catch (SQLException e) {
